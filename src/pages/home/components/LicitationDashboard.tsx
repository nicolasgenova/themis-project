...
export function LicitationDashboard({ records, municipalityId, panelOpen }: { records: PublicRecord[]; municipalityId: string; panelOpen: boolean }) {
  ...
  const explainRow = (record: PublicRecord) => selectItem({
    title: `Processo completo ${record.processNumber || record.biddingNumber || ''}`.trim(),
    value: friendlyPurpose(record),
    explanation: 'Esta linha reúne as principais informações de uma contratação. A IA pode relacionar a publicação, o número, a modalidade, o objetivo, o andamento e o valor para explicar o processo como um todo.',
    apiContext: {
      ...record.source,
      campoSelecionado: 'Linha completa do processo',
      numeroDoProcesso: record.processNumber || 'Não informado',
      dataDePublicacao: dateOnly(record.publishedAt),
      formaDeContratacao: friendlyModality(record.modality),
      objetivoSimplificado: friendlyPurpose(record),
      andamentoSimplificado: friendlyStatus(record.status),
      valorFinal: record.approvedValue === null ? 'Não informado' : formatCurrency(record.approvedValue),
    },
  })
  ...
  return <>
    ...
    <section className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b px-6 py-5"><h2 className="text-lg font-bold">Processos publicados recentemente</h2><p className="text-sm text-slate-500">Textos simplificados a partir dos dados oficiais. Clique nos títulos das colunas para ordenar. No Modo Dicionário, clique em uma linha para entender o processo inteiro ou em um item para explicar apenas aquele dado.</p></div><div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr>{columns.map((column, index) => <SortableTableHeader key={column.label} label={column.label} active={sort.column === index} direction={sort.direction} onSort={() => changeSort(index)} />)}</tr></thead><tbody className="divide-y">{shown.map((record) => <tr key={record.id} tabIndex={selectionMode ? 0 : -1} onClick={() => selectionMode && explainRow(record)} onKeyDown={(event) => { if (selectionMode && event.target === event.currentTarget && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); explainRow(record) } }} aria-label={selectionMode ? `Pedir explicação do processo completo ${record.processNumber || record.biddingNumber}` : undefined} className={selectionMode ? 'cursor-help transition hover:bg-amber-50 focus-visible:bg-amber-50 focus-visible:outline-2 focus-visible:outline-inset focus-visible:outline-amber-500' : ''}>{[
    ...
    ].map(([title, value, description], index) => <td key={title} className={`px-5 py-4 ${index === 3 ? 'max-w-sm' : ''}`}><button disabled={!selectionMode} onClick={(event) => { event.stopPropagation(); explain(record, title, value, description) }} className={`${index === 3 ? 'line-clamp-3' : ''} ${cellSelectable}`}>{value}</button></td>)}</tr>)}</tbody></table></div><TablePagination currentPage={currentPage} totalPages={totalPages} totalItems={records.length} start={start} itemsPerPage={ITEMS_PER_PAGE} itemLabel="processos" onPageChange={goToPage} /></section>
  </>
}
